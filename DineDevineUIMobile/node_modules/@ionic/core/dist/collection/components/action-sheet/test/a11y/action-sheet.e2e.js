...
const testAria = async (page, buttonID, expectedAriaLabelledBy) => {
...
const testAriaButton = async (page, buttonID, expectedAriaLabelledBy, expectedAriaLabel) => {
  const didPresent = await page.spyOnEvent('ionActionSheetDidPresent');
  const button = page.locator(`#${buttonID}`);
  await button.click();
  await didPresent.next();
...
configs({ directions: ['ltr'] }).forEach(({ config, title }) => {
  test.describe(title('action-sheet: a11y'), () => {
    ...
    test('should have aria-labelledby and aria-label added to the cancel button when htmlAttributes is set', async ({ page, }) => {
