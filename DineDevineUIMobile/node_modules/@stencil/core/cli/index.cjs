/*!
 Stencil CLI (CommonJS) v4.6.0 | MIT Licensed | https://stenciljs.com
 */
'use strict';
...
const taskGenerate = async (config) => {
    ...
    await config.sys.createDir(normalizePath(path.join(outDir, testFolder)), { recursive: true });
 ...
 * added)
 */
const getFilepathForFile = (filePath, componentName, extension) => isTest(extension)
    ? normalizePath(path.join(filePath, 'test', `${componentName}.${extension}`))
    : normalizePath(path.join(filePath, `${componentName}.${extension}`));
/**
...
const run = async (init) => {
    ...
    try {
        ...
        const foundConfig = unwrap(findConfigResults);
...
const runTask = async (coreCompiler, config, task, sys) => {
    var _a;
    const flags = createConfigFlags((_a = config.flags) !== null && _a !== void 0 ? _a : { task });
    config.flags = flags;
    if (!config.sys) {
        config.sys = sys;
    }
